...
function endpoint() {
  return `/manager/check-in/update`;
}

export function useUpdateCheckIn(options?: MutationOptions<boolean, FromCheckInWorkerModel>) {
  ...
  function handler(form: FromCheckInWorkerModel) {
    return handleCallApi(
      async () => {
        ...
        queryClient.invalidateQueries({ queryKey: ['update-check-in'] });
