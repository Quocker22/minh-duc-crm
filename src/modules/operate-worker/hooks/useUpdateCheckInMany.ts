...
function endpoint() {
  return `check-in/update-many`;
}

export function useUpdateCheckInMany(options?: MutationOptions<boolean, FromCheckInWorkerModel>) {
  ...
  function handler(form: FromCheckInWorkerModel) {
    return handleCallApi(
      async () => {
        ...
        queryClient.invalidateQueries({ queryKey: ['update-many-check-in'] });
