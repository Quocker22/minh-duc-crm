...
function endpoint() {
  return `authorization/role/update`;
}

export function useUpdateRole(options?: MutationOptions<boolean, FormRoleModel>) {
  ...
  function handler(form: FormRoleModel) {
    return handleCallApi(
      async () => {
        ...
        queryClient.invalidateQueries({ queryKey: ['update_role'] });
