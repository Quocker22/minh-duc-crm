...
import { STATUS_OPTION, StatusOption } from '@/modules/worker/models';
import moment from 'moment';

export const statusWorker = (status: string) => {
  if (!status) {
    return {
      bgColor: '#B8B8B8',
...
function getColumnDef(
  ...
  return [
        ...
        return (
          <div className="d-flex items-center gap-3">
            ...
            {row.original.worker.doc_verification_status === 'NOT_VERIFY' ? null : (
              <span className="material-symbols-outlined text-success fs-2">task_alt</span>
            )}
          </div>
        );
      },
    ...
    columnHelper.accessor('worker.birthday', {
      cell: ({ row }) =>
        row.original.worker.birthday
          ? moment(row.original.worker.birthday).format('DD/MM/YYYY')
          : null,
    ...
    columnHelper.accessor('created_by.name', {
      cell: ({ row }) => {
        ...
        return (
          <div className="d-flex items-center gap-3">
            <Avatar src={avatar} style={{ flexShrink: 0 }} />
            <div style={{ flexGrow: 1, width: '120px' }}>{row.original.created_by?.name}</div>
          </div>
        );
      },
      enableResizing: false,
    ...
    columnHelper.accessor('created_by.status', {
      cell: ({ row }) =>
