...
export const SelectOtherRequirements = <IForm extends object>(props: Props<IForm>) => {
  const { data, isLoading, isFetching } = useGetMasterList();
  const otherRequirementsList = useMemo(() => data?.find((i) => i.id === 'YEU-CAU-KHAC'), [data]);
  ...
  const selectProps = {
    ...
    name: props.name,
    options: otherRequirementsList?.data || [],
