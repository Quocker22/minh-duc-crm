...
import { defaultMenuOptions, MenuComponent } from '@/_metronic/assets/ts/components/MenuComponent';
import { IFunction } from '@/models';

export interface ISearchOptions {
  enter: boolean; // Miniam text lenght to query search // Enable search on keypress
  keypress: boolean;
  ...
  responsive?: number; // Always show menu on input focus
}
...
const defaultSearchOptions: ISearchOptions = {
  ...
  // Enable search on enter key press
  layout: 'menu',
  minLength: 2, // Use 'menu' or 'inline' layout options to display search results
...
class SearchComponent {
  ...
  private _getResponsiveFormMode = () => {
    const responsive = this._getOption('responsive') as string;
  ...
  private _focus = () => {
    ...
    if (
      this._getOption('show-on-focus') === true ||
  ...
  // Enter
  private _enter = (e: KeyboardEvent) => {
  ...
  private _input = () => {
    if (this._getOption('min-length')) {
      const minLength = parseInt(this._getOption('min-length') as string);
  ...
  private _handlers(): void {
    ...
    if (this._getOption('keypress') === true) {
      this.inputElement.addEventListener('input', this._input);
    }
    ...
    if (this._getOption('enter') === true) {
      this.inputElement.addEventListener('keypress', this._enter);
    }
    ...
    if (this.menuObject) {
      ...
      if (this.toggleElement) {
        ...
        this.menuObject.on('kt.menu.dropdown.show', () => {
        ...
        this.menuObject.on('kt.menu.dropdown.hide', () => {
          if (isVisibleElement(this.toggleElement)) {
  ...
  public update = () => {
    ...
    if (this.layout === 'menu') {
      const responsiveFormMode = this._getResponsiveFormMode();
